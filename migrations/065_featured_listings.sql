...
ALTER TABLE agents ADD COLUMN IF NOT EXISTS featured INTEGER DEFAULT 0;
ALTER TABLE agents ADD COLUMN IF NOT EXISTS featured_until INTEGER;
...
CREATE TABLE IF NOT EXISTS featured_agents (
  ...
  price_cents INTEGER DEFAULT 20000, -- $200/month
...
CREATE TABLE IF NOT EXISTS featured_slots (
  id INTEGER PRIMARY KEY,
  slot_number INTEGER NOT NULL UNIQUE, -- 1-10 slots
...
INSERT OR IGNORE INTO featured_slots (slot_number, price_cents, status) VALUES
  (1, 20000, 'available'),
  (2, 20000, 'available'),
  (3, 20000, 'available'),
  (4, 20000, 'available'),
