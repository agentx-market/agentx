...
function sendWelcomeBonus(recipientId, amount_sats = WELCOME_BONUS_SATS, description = 'welcome bonus') {
  try {
    const safeDescription = String(description).replace(/"/g, '\\"');
    const output = execLwCommand(`lw send ${recipientId} ${amount_sats} "${safeDescription}"`);
    console.log(`[wallet] Sent ${amount_sats} sats to recipient ${recipientId}`);
