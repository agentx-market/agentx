...
function normalizePostMeta(meta = {}) {
  ...
  const faqItems = parseJsonValue(meta.faq_json, []);
  const featuredImage = meta.featured_image || '';
  const socialImage = meta.social_image || featuredImage || '';
  ...
  return {
    ...
    keywords,
    relatedSlugs,
    faqItems: Array.isArray(faqItems) ? faqItems.filter((item) => item && item.question && item.answer) : [],
    featuredImage,
    socialImage,
