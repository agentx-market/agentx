...
async function findOrCreateOperator(provider, providerData) {
  ...
  if (provider === 'github' && githubAccountCreatedAt) {
    const accountAgeMs = Date.now() - githubAccountCreatedAt;
  ...
  if (provider === 'github' && createdOperator && operator.wallet_id && !operator.welcome_bonus_claimed_at) {
    try {
      lwWallet.sendWelcomeBonus(operator.wallet_id, lwWallet.WELCOME_BONUS_SATS, 'oauth signup bonus');
      db.run(
      ...
      operator.updated_at = now;
      console.log(`[oauth] Sent ${lwWallet.WELCOME_BONUS_SATS} sats to operator ${operator.id} after GitHub signup`);
    } catch (err) {
      console.error(`[oauth] Failed to send welcome bonus to operator ${operator.id}:`, err.message);
    }
  }
