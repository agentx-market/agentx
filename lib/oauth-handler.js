...
async function findOrCreateOperator(provider, providerData) {
  ...
  if (!operator) {
    ...
    if (email && !operator.welcome_email_sent) {
      try {
        await email.sendWelcome(email, name);
        db.run('UPDATE operators SET welcome_email_sent = 1 WHERE id = ?', [operator.id]);
        operator.welcome_email_sent = 1;
        console.log(`[oauth] Welcome email sent to ${email}`);
      } catch (err) {
        console.error(`[oauth] Failed to send welcome email to ${email}:`, err.message);
      }
    }
  } else {
